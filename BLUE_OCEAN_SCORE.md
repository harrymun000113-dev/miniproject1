...
| 관세 데이터 (관세율, FTA 여부) | 관세청 Open API (승인 지연 가능성 있음) |
| 물류 데이터 (운송거리/기간) | 자체 계산 또는 근사치 사용 |

각 데이터 소스는 실제 API 사용 가능 여부와 라이선스를 확인한 후 확정합니다. 확보되지 않은 지표(관세·물류·Google Trends 등)는 소수 국가만 하드코딩하거나, 1차 버전에서는 제외하고 향후 확장 항목으로 명시해도 됩니다.
